"""Expression effects of non-fusion variant classes.
...
def variant_expression_effect_rules_df():
...
def infer_variant_expression_hypotheses(
    sample_tpm_by_symbol: dict[str, Any] | None,
    *,
    tumor_tpm_by_symbol: dict[str, Any] | None = None,
    cancer_code: str | None = None,
) -> list[dict[str, Any]]:
    """Return expression patterns compatible with curated variant classes."""
    rules = variant_expression_effect_rules_df().fillna("")
        ...
        hypotheses.append(
            {
                ...
                # ``alteration`` is the upstream dataset's legacy column name;
                # trufflepig exposes the concept as a symbolic variant class.
                "variant_class": str(rule.get("alteration") or "").strip(),
