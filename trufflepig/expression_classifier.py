...
def mlh1_rna_observation(
    sample_tpm_by_symbol: Mapping[str, float],
) -> dict[str, float] | None:
    """Return measured bulk MLH1 clean-TPM, or None for missing/invalid input.

    The reference comparison is added where the cohort is in scope. Neither
    measurement establishes protein retention, promoter methylation, functional
    MMR or a mutation mechanism. Measured zero remains distinct from invalid data.
    """
    raw = sample_tpm_by_symbol.get("MLH1")
    if (
        not isinstance(raw, (int, float)) or isinstance(raw, bool)
        or not np.isfinite(raw) or raw < 0
    ):
        return None
    return {"tpm": round(float(raw), 3)}
    ...
    *,
    ...
    return LearnedExpressionVote(
        ...
        details={
            ...
            "mlh1_expression": mlh1_rna_observation(sample_tpm_by_symbol),
