...
@dataclass(frozen=True)
class ResidualIdentityScope:
    """Relationship between residual identity evidence and the report label.

    A parent-level residual can corroborate a report branch, but it cannot
    establish a more specific child.  This contract keeps that distinction
    identical in every report surface instead of letting each renderer infer
    it independently from loosely related analysis fields.
    """
    ...
    refit_accepted: bool
    selected_by_consensus: bool
    consensus_decisive: bool
    ...
    def from_analysis(
        ...
        residual = analysis.get("residual_identity_evidence") or {}
        selected = (
            (analysis.get("cancer_type_evidence") or {}).get("selected")
            or {}
        )
        consensus = selected.get("entity_evidence_consensus") or {}
        residual_code = _clean(residual.get("candidate_code"))
        return cls(
            report_code=report,
            residual_code=residual_code,
            relationship=cancer_type_tree_relationship(report, residual_code),
            source_resolved=bool(
                residual_code and residual.get("source_resolved_identity")
            ),
            refit_accepted=bool(
                (analysis.get("post_residual_decomposition_refit") or {}).get(
                    "accepted"
                )
                is True
            ),
            ...
            consensus_decisive=bool(
                consensus.get("source_resolved_identity_decisive")
            ),
        )

    @property
    def is_selection_basis(self) -> bool:
        """Whether this exact residual result established the report label."""

        return bool(
            self.relationship == "same"
            and self.source_resolved
            and self.refit_accepted
            and self.selected_by_consensus
            and self.consensus_decisive
        )

    @property
    def is_confirmed_context(self) -> bool:
        """Whether the final refit retained a source-resolved residual scope."""
