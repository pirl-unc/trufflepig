"""Regression tests for the NET_PANCREAS cohort integration (part of #152).
...
as a new ``cancer_code=NET_PANCREAS`` block.
...
def test_pannet_registry_points_at_alvarez_cohort():
    ...
    row = reg[reg["code"] == "NET_PANCREAS"].iloc[0]
...
def test_pannet_canonical_ne_markers_are_high():
    ...
    sub = subtype_deconvolved_expression()
    pannet = sub[sub["cancer_code"] == "NET_PANCREAS"].set_index("symbol")
...
def test_pannet_lineage_panel_present():
    ...
    assert "NET_PANCREAS" in panels, "NET_PANCREAS lineage panel not registered"
    panel = set(panels["NET_PANCREAS"])
    # Canonical NE markers must anchor the panel — any future edit
    # that drops these needs to be explicit.
    for gene in ("CHGA", "SYP", "ENO2"):
        assert gene in panel, f"NET_PANCREAS lineage panel missing {gene}"
...
def test_pannet_panel_loaded_into_LINEAGE_GENES():
    ...
    estimator should carry the NET_PANCREAS panel."""
    assert "NET_PANCREAS" in LINEAGE_GENES
    assert "CHGA" in LINEAGE_GENES["NET_PANCREAS"]
