...
_MISSING_THERAPY_AXIS = frozenset(
    {
        ...
        "UVM",
        "WILMS",
        # NCI-coverage expansion (pirlygenes 5.18) — no therapy-axis panel yet.
        "ANSC",
        "BCC",
        "cSCC",
        "GBC",
...
_TOLERATED_GAPS_EXPLICIT = {
    ...
    "NET_LUNG": {"expression"},
    "NEC_LUNG_LARGECELL": {"expression", "lineage", "biomarker", "therapy"},
    "NET_MIDGUT": {"expression"},
    # Rectal NET: new leaf in pirlygenes >=5.11 with only an expression
    # reference so far (no lineage panel / biomarker / therapy data yet).
    "NET_RECTAL": {"lineage", "biomarker", "therapy"},
    "NEC_MERKEL": {"expression"},
    "MTC": {"expression"},
