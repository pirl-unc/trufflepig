"""Tests for the non-HK signal views and decision reasoning.
...
def _views(tpm, within=None):
    return compute_views(
        "sig", GENES, tpm, within_sample_pct=within, cohort_reference=COHORT
    )


def test_all_non_hk_views_are_computed():
...
def test_concordance_low_when_views_disagree():
    ...
    big_ref = {g: np.array([1000.0] * 10) for g in GENES}
    r = signal_report("sig", GENES, {"A": 450.0, "B": 450.0, "C": 450.0},
...
def test_reason_over_views_handles_missing_views():
    r = reason_over_views("sig", {"within_pct": 0.9, "log1p": 6.0,
